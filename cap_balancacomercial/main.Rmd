...
# Produtos exportados
read_xlsx("cap_balancacomercial/cap_balancacomercialbc_2023.xlsx", sheet = "produtos_export") %>%
...
# Produtos importados
read_xlsx("cap_balancacomercial/cap_balancacomercialbc_2023.xlsx", sheet = "produtos_import")  %>%
  ggplot(aes(data, valor, fill = produto)) +
  geom_bar(stat = "identity", position = "dodge", gap = "s") +
  scale_y_continuous(labels = function(x) x / m) +
...
c("export", "import", "saldo") %>%
  purrr::map(
    ~readxl::read_xlsx("cap_balancacomercial/cap_balancacomercialbc_2023.xlsx", sheet = .x) %>%
