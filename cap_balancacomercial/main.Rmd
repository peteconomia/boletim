...
# Produtos exportados
readxl::read_excel("cap_balancacomercial/bc_2020.xlsx", sheet = "produtos_export") %>%
...
# Produtos importados
readxl::read_excel("cap_balancacomercial/bc_2020.xlsx", sheet = "produtos_import") %>%
  ggplot(aes(data, valor, fill = produto)) +
  geom_bar(stat = "identity", position = "dodge", gap = "s") +
  scale_y_continuous(labels = function(x) x / m) +
...
c("export", "import", "saldo") %>%
  purrr::map(
    ~readxl::read_excel("cap_balancacomercial/bc_2020.xlsx", sheet = .x) %>%
...
dplyr::bind_cols(
  ...
  kbl(
    ...
    caption = "Origem das Exmportações e Importações\\label{tab:import_export}",
    # label = "import_export",
    centering = FALSE
  ) %>%
  footnote(
